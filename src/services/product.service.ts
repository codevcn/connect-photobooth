...
class ProductService {
  ...
  private async fetchProducts(page: number, pageSize: number): Promise<TBaseProduct[]> {
    ...
    for (const product of apiProducts) {
      ...
      const category = product.attributes_json.category as TClientProductVariant['category']
      ...
      // Convert each variant to TClientProductVariant (without printAreaInfo)
      const variants: TClientProductVariant[] = product.variants.map(
